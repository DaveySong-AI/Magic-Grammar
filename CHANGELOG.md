...
### 语法题库（Issue #57）
- 📚 首页新增「题库」入口（与错题本并列）；题库总览页按 22 章卡片展示「章节名 + 知识点 + 题数 + 完成进度 + 正确率」，顶部汇总总题数 / 已做 / 正确率 / 完成度
- 🧩 章节测试页：每章 100 分按题数均分、逐题显示分值；选择题即时判分，自由作答题为「输入框 + 查看参考答案 + 学生自判对错」（方案A）
- 🔁 错题支持重做直至答对，重做正确标记「已纠正」；解析字段优先逐题解析、缺省用章节知识点兜底、第11章用答案表「要点」
- 📕 题库错题自动并入错题本（标注「题库·第N章」来源），须在错题本内重做正确 / 自判已掌握 / 手动删除才移出
- 💾 独立进度存储 `grammarBankProgress`，与首页通关进度互不影响；第16章「结语」标记非测试
- 🛠️ 新增 `build_question_bank.py` 从《语法俱乐部题库-修正版.md》自动生成 `question-bank.js`（439题：289 选择 + 150 自由）
- ✅ 校验：jsdom 无头 41 项功能断言全通过；真实 Chrome 交互验证 + 页面截图核验；node 语法检查通过

## [V1.4.0] - 2026-08-28

### 前3页解说词批量 TTS 重录（#34–#56 P1/P3 精简 + #45 第12课 P3/P7/P13 + #49 第17课 P2）
- 🎙️ 按《TTS待重录清单》P4/P5/P6/#33 段批量重录 **47 页**女老师讲解音频，覆盖各课 P1 封面 / P3 引入（第16、21课在 P5 段；第17课额外 P2，第12课额外 P7/P13），沿用「温柔亲切30岁女老师」音色与统一 prompt
- 🔧 47 个新音频 URL 已精确关联到 `allAudioUrls` 对应索引（P1=索引0、P3=索引2；第12课 P7=索引6、P13=索引12；第17课 P2=索引1），与 slide 顺序一一对应
- ✏️ 第17课 P2（#49）：目标三「疑问词保留」→「有无宾语、有无介系词、主语用for安插」，data-note 已改并与音频一致（第18课 P2 的「疑问词保留」为正确口径，未动）
- 📝 本次 47 页映射登记于 `tts_urls_20260828_batch.json`；《TTS待重录清单》P4/P5/P6/#33 段全部标记 ✅ 已完成
- ✅ 校验：check_quotes / check_page_refs 通过；node 语法检查通过；47/47 替换位置验证正确；数组长度与 slide 数一致

## [V1.3.2] - 2026-08-28

### 前3页解说词精简补充（#55 第16课 / #56 第21课）
- ✏️ 两课内容审核零卡片问题，仅按 workbuddy 评论【精简后文案】精简 P1 封面 / P3 引入 data-note（L16 198→91、447→206；L21 136→56、294→168）；L16 P3 命名学段依评论从解说词移除（命名卡片暂留，下移 P4 待产品决定）；L21 去 P1↔P3 67% 重复
- check_quotes ✅；P1/P3 登记《TTS待重录清单》P5 段。refs #55 #56，未关闭
